...
const VAULT_ABI = [
  ...
  "function previewMint(uint256 grossAmount, uint8 lockDays, bool assumeGenesis) view returns (uint256 peakSato, uint32 weight, uint8 rarity, bool wouldBeGenesis)",
  "function vaults(uint256) view returns (uint256 peakSato, uint256 satoLocked, uint64 lockEnd, uint32 weightAtMint, uint8 lockDays, uint8 rarity, bool isGenesis, uint8 genesisRank, bool redeemed)",
  ...
  "event StoneMinted(address indexed minter, uint256 indexed tokenId, uint256 grossSato, uint256 peakSato, uint8 lockDays, uint32 weightAtMint, bool isGenesis, uint8 genesisRank)",
];
let vaultIface;
...
const LOCK = { Fifteen: 0, Thirty: 1, Sixty: 2 };
...
async function updateMintUI() {
  ...
  try {
    ...
    const [bal, allowance, mintedByWallet, maxPerWallet, maxGross, preview] = await Promise.all([
      ...
      vault.previewMint(grossWei, lock, true),
    ]);
    if (nonce !== mintUiNonce) return;

    setText("mintSatoBal", formatSato(bal));
    const issues = [];
    if (grossWei < ethers.parseUnits("100", satoDecimals)) issues.push("minimum deposit is 100 SATO");
